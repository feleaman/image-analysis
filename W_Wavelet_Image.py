...
InputsOpt_Defaults = {'mypath':None, 'range':None, 'mother_wavelet':'morl', 'widths':[1,21], 'cmap':'Greys', 'output':'plot', 'level_wavelet':10, 'filter':['highpass', 5.e3, 3], 'absolute':'OFF', 'img_size':512}

def main(argv):
	config = read_parser(argv, Inputs, InputsOpt_Defaults)
	mydir = abspath(getcwd())
	
	if config['mode'] == 'cwt_one':
		#+++read data
		...
		im = Image.fromarray(np.uint8(cwtmatr*255))
		im = im.resize((512,512))
		im.save('mytest.png')
	
	elif config['mode'] == 'cwt_all':
		#+++read data
		if config['mypath'] == None:
			print('select signal file: ')
			root = Tk()
		...
		length = int(config['range'][1]*config['fs']) - int(config['range'][0]*config['fs'])
		
		wfm = load_signal(filepath, config['channel'])
		
		total = int(len(wfm)/length)
		
		for i in range(total-1):
		
			wfm = load_signal(filepath, config['channel'])			
			wfm = wfm[i*length : (i+1)*length]
			...
			im = Image.fromarray(np.uint8(cwtmatr*255))
			im = im.resize((config['img_size'],config['img_size']))
...
def read_parser(argv, Inputs, InputsOpt_Defaults):
	...
	config['img_size'] = int(config['img_size'])
